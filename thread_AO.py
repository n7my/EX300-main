...
class AOThread(QObject):
    ...
    def AOOption(self):
        ...
        if self.isCalibrate:
            boola = self.calibrateAO()
            if not boola:
                self.pass_signal.emit(False)

        if self.isTest:
            if self.isAOTestVol:
                if not self.testAO('AOVoltage'):
                    isExcel = False
                    self.pass_signal.emit(False)


            if self.isAOTestCur:
                if not self.testAO('AOCurrent'):
                    isExcel = False
                    self.pass_signal.emit(False)
    ...
    def AOTestLoop(self, type):
        ...
        for i in range(5):
            ...
            while True:
                ...
                bool_receiveAO, usReceValue = self.receiveAIData()
                if not bool_receiveAO:
                    self.pauseOption()
                    if not self.is_running:
                        return False
                    self.messageBox_signal.emit(['警告', '数据接收出错，请检查设备'])
                    return False
                elif bool_receiveAO == 'stopReceive':
                    return False
                else:
                    self.pauseOption()
                    if not self.is_running:
    ...
    def receiveAIData(self):
        ...
        for i in range(self.m_Channels):
            ...
            ## data = b'\{self.m_can_obj.Data[i * 2]\self.m_can_obj.Data[i * 2 + 1]'
    ...
    def calibrateAO(self):
        if self.isCalibrateVol == True:
            ...
            bool_calibrate = self.calibrateAO_vol_cur('AOVoltage')
        ...
        if self.isCalibrateCur == True:
            ...
            bool_calibrate = self.calibrateAO_vol_cur('AOCurrent')
            if not bool_calibrate:
                return False
            self.pauseOption()
            if not self.is_running:
    ...
    def calibrateAO_vol_cur(self, type):
        # if type == 'AOVoltage':
        #     pass
        maxRange = 0
        lowValue = 0
        ...
        bool_receive, usArrayHigh = self.receive_WriteToAO(highValue, type, channelNum)
        if not bool_receive:
            return False
        self.pauseOption()
        if not self.is_running:
        ...
        time.sleep(2)
        bool_receive, usArrayLow = self.receive_WriteToAO(lowValue, type, channelNum)
        if not bool_receive:
            return False
    ...
    # 计算零值
    def calcZero(self, usHighValue, usLowValue, m_highValue, m_lowValue, type):
        if type == 'AOVoltage':
    ...
    def receive_WriteToAO(self, value, type, channelNum):
        ...
        while True:
            if time.time() - tt1 >= intWait:
                break
            # self.receiveAIData()
            CAN_option.receiveCANbyID(0x280 + self.CANAddr_AI, self.waiting_time)
            time.sleep(0.1)
        ...
        for i in range(self.receive_num):
            ...
            while True:
                ...
                time.sleep(0.1)
                notReceive = False
                bool_caReceive, usTmpValue = self.receiveAIData()
                    ...
                    if bool_receive:
                        ...
                        if hex(self.m_can_obj.Data[4]) == hex(self.AORangeArray[10]) and hex(
                            ...
                            self.result_signal.emit(
                                f'{AOChannel}.未成功设置AO通道{AOChannel}的量程为”电流(0-20)mA“。\n\n')
                    ...
                    if bool_receive:
                        ...
                        if hex(self.m_can_obj.Data[4]) == hex(self.AIRangeArray[10]) and hex(
                            ...
                            if not self.is_running:
                                return False
                            self.result_signal.emit(
                                f'{AIChannel}.未成功设置AI通道{AIChannel}的量程为”电流(0-20)mA“。\n\n')
            ...
            for i in range(5):
                for j in range(self.AO_Channels):
                    ...
                    if abs(self.volPrecision[i][j]) < 1:
                        ...
                        self.errorNum += 1
                        self.errorInf += f'\n{self.errorNum})测试点{i + 1}AO通道{j + 1}电压精度超出范围 '
            ...
            for i in range(5):
                for j in range(self.AO_Channels):
                    ...
                    if abs(self.curPrecision[i][j]) < 1:
                        ...
                        self.errorNum += 1
                        self.errorInf += f'\n{self.errorNum})测试点{i + 1}AO通道{j + 1}电流精度超出范围 '
                        sheet.write(self.AO_row + 2 + self.AO_Channels + j, 5 + 3 * i + 1,
                                    f'{self.curPrecision[i][j]}‰', fail_style)
        if not self.isAOTestVol and self.isAOTestCur:
            all_row = 9 + 4 + 4 + (2 + self.AO_Channels) + 2  # CPU + DI + DO + AI + AO
